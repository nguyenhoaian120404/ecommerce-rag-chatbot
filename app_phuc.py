...
# CONFIG
# =============================================
MODEL_PATH  = "hoaian12/bge-m3-legal-vn"
FAISS_INDEX = "step6_faiss_final/index.faiss"
FAISS_META  = "step6_faiss_final/chunks_meta.pkl"

# Tốc độ: giảm max_seq_length vì query thường ngắn (< 256 tokens)
MAX_SEQ_LEN  = 256
TOP_K        = 10
MAX_CHARS_PER_CHUNK = 1500
MAX_TOTAL_CHARS     = 12000

# OpenAI model — gpt-4o-mini nhanh & rẻ, có thể đổi sang gpt-4o cho chất lượng cao hơn
OPENAI_MODEL = "gpt-4o-mini"
OPENAI_TIMEOUT = 60  # giây — không treo vô hạn nếu mạng/API chậm
...
def load_resources():
    # Dùng GPU nếu có, fp16 trên GPU để encode nhanh hơn 2-3x
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = SentenceTransformer(MODEL_PATH, device=device)
    model.max_seq_length = MAX_SEQ_LEN
    if device == "cuda":
        model = model.half()  # fp16 chỉ trên GPU
    model.eval()

    index = faiss.read_index(FAISS_INDEX)
    # Tăng số thread FAISS để search nhanh hơn trên CPU đa nhân
    try:
        faiss.omp_set_num_threads(max(1, (os.cpu_count() or 4) - 1))
    except Exception:
        pass
    ...
    if not api_key:
        raise RuntimeError(
            "Thiếu OPENAI_API_KEY. Đặt trong .streamlit/secrets.toml hoặc biến môi trường."
        )
    client = OpenAI(api_key=api_key, timeout=OPENAI_TIMEOUT)
    log.info(f"✅ Loaded model on {device}, index size={index.ntotal}, meta={len(meta)} chunks")
    return model, index, meta, client


# =============================================
# EMBED QUERY — cache kết quả encode cho câu hỏi lặp
# =============================================
...
def _get_model_singleton():
    # Helper để cache_data có thể gọi model từ cache_resource
...
def retrieve(query, model, index, meta, top_k=TOP_K):
    ...
    # Giảm hệ số over-fetch từ 4x xuống 3x — đủ để dedup + balance
...
def generate_answer_stream(query, context, client):
    """Streaming generator — hiển thị từng token, cảm giác nhanh hơn nhiều."""
    user_msg = f"""===== CÁC ĐOẠN VĂN BẢN LIÊN QUAN =====
{context}
========================================
...
.src-header { display: flex; align-items: center; flex-wrap: wrap; gap: 6px; }
.src-path { font-size: 0.78rem; color: #94a3b8; margin-top: 6px; font-weight: 500; }
.src-body {
...
.src-body h1, .src-body h2, .src-body h3, .src-body h4 {
    color: #e2e8f0; margin: 0.6em 0 0.3em;
    font-size: 0.9rem; font-weight: 600;
}
.src-body ul, .src-body ol { margin: 0.3em 0 0.3em 1.2em; padding: 0; }
.src-body li { margin: 0.2em 0; }
.src-body code {
    background: rgba(99,102,241,0.15); color: #a5b4fc;
    padding: 1px 5px; border-radius: 4px; font-size: 0.85em;
}
.src-body strong { color: #f1f5f9; }
.src-body em { color: #e2e8f0; }
.src-body blockquote {
...
.src-body th, .src-body td {
    border: 1px solid #334155; padding: 4px 8px;
    text-align: left;
}
.src-body th { background: rgba(99,102,241,0.1); color: #e2e8f0; }
.src-body a { color: #818cf8; text-decoration: underline; }
.score-pill {
...
# Load
try:
    model, index, meta, client = load_resources()
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        if msg.get("sources"):
            with st.expander(f"📎 {len(msg['sources'])} nguồn tham chiếu — click để xem nội dung trích dẫn"):
                for i, src in enumerate(msg["sources"], 1):
                    ...
                    # Header: badge + score + citation path
                    st.markdown(
                        f'<div class="src-row">'
                    ...
                    # Nội dung trích dẫn — convert markdown → HTML rồi nhúng vào div có style
